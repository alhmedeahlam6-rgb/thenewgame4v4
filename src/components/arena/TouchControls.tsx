      ...
      <ControlWrap {...wrap} id="medkits" anchor="bottom-[54px] left-1/2" origin="bottom center" centerX>
        <div className="flex gap-2">
          {Array.from({ length: 3 }, (_, index) => {
            const filled = index < kits;
            return (
              <button key={index} aria-label="Use medkit" disabled={!filled && !editing} className={`${disc} h-9 w-9 ${filled ? "" : "opacity-30"}`} {...tap(() => filled && onHeal())}>
                <img src={medkitIcon} alt="" className={`h-4 w-4 ${glyph}`} />
              </button>
            );
          })}
        </div>
